 ...
 * Manages up to 5 concurrent WhatsApp sessions
 */
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        ...
        try {
          ...
          db.updateSessionStatus.run('waiting_qr', sessionName);

          broadcastWS({
            type: 'qr',
            data: { name: sessionName, qr: qrDataUrl }
          });
        } catch (err) {
          console.error(`[WA] QR generation error for ${sessionName}:`, err.message);
        }
      }
